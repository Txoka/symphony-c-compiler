...
def run_selfhost_benchmark(max_steps):
...
def main():
    ...
    selfhost = run_selfhost_benchmark(args.max_steps)
    ...
    lines += [
        ...
        "",
        "This measures the compiler generated by the Python implementation (stage 0) compiling the complete self-hosted C compiler project into stage 1. The runtime value is native-emulator target instruction steps, not host wall-clock time or a hardware cycle estimate.",
        "",
        "| Python-produced compiler bytes | Self-compiled output bytes | Compilation instructions |",
        "|---:|---:|---:|",
        f"| {cell(selfhost[0])} | {cell(selfhost[1])} | {cell(selfhost[2])} |",
    ]
    if notes:
