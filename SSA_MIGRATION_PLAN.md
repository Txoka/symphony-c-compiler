...
- [x] `eliminate_tail_calls`, `lower_self_tail_calls_to_loops`
      (`symphony/middle/ssa/tail.py`) — canonical call/return pairs become
      tail terminators; direct self tails become jumps to a new SSA loop
      header whose parameter phis carry the next iteration's arguments.
